...
def usb_calibrate_gyr_and_acc(socket):
    socket.write(b'\xFF\xAA\x01\x01\x00')
    for i in range(3, -1, -1):
        print(f"\rКалибровка гироскопа и акселерометра завершится через {i} сек...", end="")
        time.sleep(1)
    socket.write(b'\xFF\xAA\x01\x00\x00')    
    print("\nКалибровка гироскопа и акселерометра завершена.\n")  

# Калибровка магнитрометра для USB
def usb_calibrate_magn(socket, device):
    if device[0] == 'WT901BLE':
        socket.write(b'\xFF\xAA\x01\x07\x00')
    elif device[0] == 'BWT901':
        socket.write(b'\xFF\xAA\x01\x02\x00')
